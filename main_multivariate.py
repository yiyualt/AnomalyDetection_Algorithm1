...
from models_multivariate import (
    ThreeSigmaModel,
    KDEModel,
    OCSVMModel,
    EnsembleModel,
...
def make_lag_matrix(values: np.ndarray, lag_window: int) -> np.ndarray:
    """将一维序列转为时滞特征矩阵：
    输入 x0..x{n-1}，输出形状为 (n, lag_window)，第t行=[x_{t-lw+1}..x_t]，前 (lag_window-1) 行无法构造，将用NaN填充，外层应丢弃。
    """
    v = np.asarray(values, dtype=float).reshape(-1)
    n = v.size
    lw = int(lag_window)
    X = np.full((n, lw), np.nan, dtype=float)
    for t in range(lw-1, n):
        X[t, :] = v[t-lw+1:t+1]
    return X


def infer_points(win_df: pd.DataFrame, raw_per_kpi: dict, model: BaseAnomalyModel, max_windows: int | None = None, show_progress: bool = True, lag_window: int = 1):
    y_true_points: List[int] = []
    y_pred_points: List[int] = []

    it = win_df.iterrows()
    if show_progress:
        it = tqdm(it, total=len(win_df), desc=f'Running windows (lag={lag_window})')
    for idx, row in it:
        ...
        if lag_window <= 1:
            ...
            # 构造滞后特征矩阵
            HX = make_lag_matrix(H['value'].values, lag_window)
            PX = make_lag_matrix(P['value'].values, lag_window)
            # 丢弃含NaN的首部行：H需要完整窗口用于拟合；P仅保留完整行用于预测
            HX = HX[lag_window-1:, :]
            PX = PX[lag_window-1:, :]
            # 若P有效行数 < WIN_P - (lag_window-1)，则在末尾用最后一行重复补全，保持点数对齐
            if PX.shape[0] < WIN_P:
                pad = np.repeat(PX[-1:, :], WIN_P - PX.shape[0], axis=0)
                PX = np.vstack([PX, pad])
            flags = model.fit(HX).predict(PX)
...
def main():
    parser = argparse.ArgumentParser(description='Run anomaly models with optional lag features (point-level evaluation)')
    ...
    parser.add_argument('--lag_window', type=int, default=1, help='lag window size for multivariate features (e.g., 3/5/10)')
    # 3sigma
    ...
    print('LAG_WINDOW:', args.lag_window)
    ...
    y_true, y_pred = infer_points(win_df, raw_per_kpi, model, max_windows=args.max_windows, show_progress=(not args.no_progress), lag_window=args.lag_window)
